...
export default function splash({ name, logo, active, total, channel, iframe }){
  let div = dom('.splash',
    ...
    dom('p.status',
      active
        ? [
          dom('b.active', active), ' usuarios online ahora de los ',
          dom('b.total', total), ' registrados.'
        ]
        : [dom('b.total', total), ' usuarios registrados.']
    ),
    dom('form',
      dom('input type=text placeholder=tu@dominio.com '
        + (!iframe ? 'autofocus' : '')),
      dom('button.loading', 'Enviar')
    ),
